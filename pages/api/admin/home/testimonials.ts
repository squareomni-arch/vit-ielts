...
export default async function handler(
  ...
  const sectionName = "home/testimonials";

  if (req.method === "GET") {
    try {
      const config = await readConfig<TestimonialsConfig>(supabaseAdmin, sectionName);
  ...
  if (req.method === "POST") {
    try {
      ...
      await writeConfig<TestimonialsConfig>(supabaseAdmin, sectionName, body);
