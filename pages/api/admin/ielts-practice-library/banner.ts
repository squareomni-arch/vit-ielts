...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET") {
    try {
      const config = await readConfig<PracticeLibraryBannerConfig>(supabaseAdmin, sectionName);
  ...
  if (req.method === "POST") {
    try {
      ...
      await writeConfig<PracticeLibraryBannerConfig>(supabaseAdmin, sectionName, body);
