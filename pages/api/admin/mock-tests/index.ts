...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === "GET") {
        try {
            ...
            let query = supabaseAdmin
                .from("mock_tests")
                .select("id, title, slug, practice_tests, created_at", { count: "exact" });
            ...
            console.error("[API /api/admin/mock-tests] GET", error);
    ...
    if (req.method === "POST") {
        try {
            ...
            const slug = baseSlug ? `${baseSlug}-${uniqueSuffix}` : `mock-test-${uniqueSuffix}`;

            const { data, error } = await supabaseAdmin
                .from("mock_tests")
                .insert({ title, slug, practice_tests: [] })
                .select()
                .single();
            ...
            console.error("[API /api/admin/mock-tests] POST", error);
            return res.status(500).json({
                success: false,
                error: error instanceof Error ? error.message : "Internal error",
            });
