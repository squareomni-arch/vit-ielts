...
export default async function handler(
  ...
  if (req.method === "GET") {
    try {
      const config = await readConfig<SubscriptionBannerConfig>(supabaseAdmin, sectionName);
  ...
  if (req.method === "POST") {
    try {
      ...
      await writeConfig<SubscriptionBannerConfig>(supabaseAdmin, sectionName, body);
