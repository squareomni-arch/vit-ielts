...
function toIPracticeSingle(
  ...
  return {
    ...
    quizFields: {
      ...
      scoreType: [
        (quiz.score_type ?? "band") as "band" | "percentage",
        quiz.score_type ?? "band",
      ],
      ...
      passages: (quiz.passages ?? []).map((p) => ({
        ...
        start_question_number: p.start_question_number
          ? Number(p.start_question_number)
          : undefined,
        questions: (p.questions ?? []).map((q) => {
          ...
          return {
            question_form: [
              (q.question_form ??
                "uncategorized") as IPracticeSingle["quizFields"]["passages"][0]["questions"][0]["question_form"][0],
            ...
            list_of_questions: Array.isArray(listOfQuestions)
              ? listOfQuestions.map((lq) => ({
                  ...
                  correct:
                    lq.correct != null
                      ? typeof lq.correct === "string"
                        ? parseInt(lq.correct, 10) || 0
                        : Number(lq.correct)
                      : 0, // WP ACF migration: null means 0 (first option, lost due to JS falsy)
                  options: (Array.isArray(lq.options) ? lq.options : []).map(
                    (o: any) => ({
            ...
            matchingQuestion: matchingQ
              ? {
                  layoutType: (() => {
                    const lt =
                      matchingQ.layout_type ?? matchingQ.layoutType ?? null;
                    return Array.isArray(lt) ? lt[0] : lt;
                  })(),
                  summaryText:
                    matchingQ.summary_text ?? matchingQ.summaryText ?? "",
                  ...
                  answerOptions: (Array.isArray(matchingQ.answer_options)
                    ...
                    : Array.isArray(matchingQ.answerOptions)
                      ? matchingQ.answerOptions
                      : []
                  ).map((ao: any) => ({
            ...
            matrixQuestion: matrixQ
              ? {
                  matrixCategories: (Array.isArray(matrixQ.matrix_categories)
                    ...
                    : Array.isArray(matrixQ.matrixCategories)
                      ? matrixQ.matrixCategories
                      : []
                  ).map((mc: any) => ({
                    categoryLetter:
                      mc.category_letter ?? mc.categoryLetter ?? "",
                  ...
                  matrixItems: (Array.isArray(matrixQ.matrix_items)
                    ...
                    correctCategoryLetter:
                      mi.correct_category_letter ??
                      mi.correctCategoryLetter ??
                      "",
                  })),
                  layoutType: (() => {
                    const lt =
                      matrixQ.layout_type ?? matrixQ.layoutType ?? "standard";
                    return Array.isArray(lt) ? lt[0] : lt;
                  })(),
                  legendTitle:
                    matrixQ.legend_title ?? matrixQ.legendTitle ?? "",
                }
              : undefined,
          };
        }),
